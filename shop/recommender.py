...
class Recommender:
    ...
    def suggest_products_for(self, products, max_results=6):
        ...
        if len(products) == 1:
            ...
            #multpile products, comine scotes of all products
            # sotre the resulting sorted set in a temporaty key
            keys = [self.get_product_key(id) for id in products_ids]
            r.zunionstore(tmp_key, keys)
            #remove ids for the products in recommendation is for 
            r.zrem(tmp_key, *products_ids)
            # get the product ids bu their score, descent sort
